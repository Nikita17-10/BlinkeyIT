...
const AddToCartButton = ({ data }) => {
  ...
  useEffect(() => {
    ...
    // 🔒 Safe filter — skip null items
    const validCart = cartItem.filter(item => item && item.productId && item.productId._id)

    // ✅ Check if product already in cart
    const isPresent = validCart.some(item => item.productId._id === data._id)
    setIsAvailableCart(isPresent)

    // ✅ Find product details
    const foundProduct = validCart.find(item => item.productId._id === data._id)
    setQty(foundProduct?.quantity || 0)
    setCartItemsDetails(foundProduct || null)
