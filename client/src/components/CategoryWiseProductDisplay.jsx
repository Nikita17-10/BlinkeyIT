...
const CategoryWiseProductDisplay = ({ id, name }) => {
  ...
  useEffect(() => {
    fetchCategoryWiseProduct();
  }, [id]); // ✅ refetch when category id changes
  ...
  const handleRedirectProductListpage = () => {
    if (!id || !subCategoryData?.length) return "/";

    const subcategory = subCategoryData.find((sub) =>
      sub.category?.some((c) => c._id === id)
    );

    if (!subcategory) return `/category/${valideURLConvert(name)}-${id}`;

    const categoryURL = valideURLConvert(name);
    const subCategoryURL = valideURLConvert(subcategory.name);

    return `/${categoryURL}-${id}/${subCategoryURL}-${subcategory._id}`;
  };
      ...
      <div className="relative flex items-center">
        ...
        >
          ...
          {!loading && data.length > 0
            ...
            : !loading && (
                <p className="text-gray-500 px-4">No products found</p>
              )}
        </div>

        <div className="w-full absolute hidden lg:flex justify-between px-2">
          <button
