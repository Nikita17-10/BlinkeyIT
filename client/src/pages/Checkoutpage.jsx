...
const CheckoutPage = () => {
  ...
  try {
    ...
    // 2️⃣ If backend returns Stripe Checkout URL, redirect there
    if (responseData?.url) {
      window.location.href = responseData.url; // ✅ Go to Stripe Checkout
    } else {
      toast.error("Unable to start payment — no Stripe URL received.");
      console.error("Stripe response:", responseData);
    }

    // 3️⃣ Optional refresh (if user cancels and returns)
